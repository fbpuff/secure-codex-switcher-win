...
export const USAGE_WINDOW_SECONDS = Object.freeze({
  fiveHour: 18_000,
  oneWeek: 604_800
});

export const USAGE_AUTH_EXPIRED_MESSAGE = "当前保存的用量认证已过期（401），账号仍可切换。请切换到该账号并打开官方 Codex，待认证自动刷新后再试；若仍失败，再重新登录并导入。";
...
export async function fetchUsageSnapshot({ accessToken, accountId, fetchImpl = fetch, now = unixNow }) {
  ...
  for (const url of usageEndpoints) {
    try {
      ...
      return { ...mapUsagePayload(await response.json(), now()), source: "chatgpt_usage_api" };
    } catch (error) {
...
export function mapUsagePayload(payload, fetchedAt = unixNow()) {
  ...
  const fiveHour = pickExactWindow(windows, USAGE_WINDOW_SECONDS.fiveHour);
  const oneWeek = pickExactWindow(windows, USAGE_WINDOW_SECONDS.oneWeek);
...
function parseUsageWindowRaw(value) {
  ...
  const usedPercent = normalizePercent(value.used_percent);
...
function pickExactWindow(windows, targetSeconds) {
  return windows.find((window) => window.limitWindowSeconds === targetSeconds);
}

function toUsageWindow(raw) {
  const remainingPercent = normalizePercent(100 - raw.usedPercent);
  return {
...
function summarizeFailures(failures) {
  ...
  if (failures.some((failure) => /401/i.test(failure.message))) {
    return USAGE_AUTH_EXPIRED_MESSAGE;
  }
