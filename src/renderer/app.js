...
const messages = {
  "zh-CN": {
    ...
    "status.autoSwitched": "当前账号已达到自动切换条件，已切换到 {email}，并关闭 {count} 个 ChatGPT Codex 进程。",
    "status.autoSwitchQueued": "当前账号已达到自动切换条件，已排队切换到 {email}。检测到正在运行或刚结束的 ChatGPT Codex 对话/任务，短暂等待后将自动切换并重新打开。",
    "status.autoSwitchFallbackQueued": "指定账号不可用（{reason}），本次已按综合评分排队切换到 {email}。",
    "status.autoSwitchFallbackSwitched": "指定账号不可用（{reason}），本次已按综合评分切换到 {email}，并关闭 {count} 个 ChatGPT Codex 进程。",
    ...
    "status.accountNeedsLogin": "{email} 需要重新登录。请切换到该账号并重新打开 ChatGPT Codex，刷新登录态后再导入/刷新。",
  ...
  en: {
    ...
    "status.autoSwitched": "Current account reached an auto-switch condition; switched to {email} and closed {count} ChatGPT Codex processes.",
    ...
    "status.autoSwitchFallbackQueued": "The selected target is unavailable ({reason}); queued highest composite-score account {email} for this switch.",
    "status.autoSwitchFallbackSwitched": "The selected target is unavailable ({reason}); switched to highest composite-score account {email} and closed {count} ChatGPT Codex processes.",
    ...
    "status.accountNeedsLogin": "{email} needs login refresh. Switch to that account, reopen ChatGPT Codex, then import/refresh again.",
...
function initializeReportsWeek() {
  ...
  reportsWeekInput.max = reportDates.daily;
...
function showView(view) {
  currentView = view === "settings" || view === "usage" || view === "reports" ? view : "accounts";
...
function renderMetrics() {
  ...
  const best = [...accounts].sort((left, right) => compareAccountsByScore(left, right))[0];
...
function formatScoreComponent(value) {
  return Number.isFinite(value) ? String(Math.round(value)) : t("labels.unknown");
}
...
async function syncAutoSwitchTargetState() {
...
async function handleAutoSwitchEvaluation(result) {
  if (!result || result.status === "disabled" || result.status === "idle" || result.status === "not_ready") {
    pendingAutoSwitch = (await api.getAutoSwitchState()).pending;
    renderQuotaWarning();
    return;
  }
  ...
  if (result.status === "queued") {
    ...
    startPendingAutoSwitchTimer();
    const fallback = result.fallback ?? result.pending?.fallback;
    setStatus(fallback
      ? t("status.autoSwitchFallbackQueued", {
          email: result.target?.emailMasked ?? result.pending?.emailMasked ?? t("labels.unknown"),
          reason: fallbackReasonLabel(fallback.reason)
        })
      : t("status.autoSwitchQueued", { email: result.target?.emailMasked ?? result.pending?.emailMasked ?? t("labels.unknown") }));
    renderQuotaWarning();
    return;
  }
...
async function finishAutoSwitch(target, switchResult, fallback) {
  ...
  try {
    ...
    const message = fallback
      ...
      : t("status.autoSwitched", { email: target?.emailMasked ?? t("labels.unknown"), count: switchResult?.closedCodexProcesses ?? 0 });
    ...
    <section class="detail-section">
      ...
      <div class="reset-card">
        ...
        <div class="detail-recovery-row">
          ...
          <strong>${account.usage ? `${formatScore(quotaBalanceScore(account))} ${t("labels.scoreUnit")}` : t("labels.unknown")}</strong>
        </div>
        <div class="detail-recovery-row">
      ...
      <div class="quota-provenance">
        <span>${escapeHtml(t("detail.quotaSource", { source: account.usage?.source === "chatgpt_usage_api" ? "ChatGPT/Codex usage API" : t("labels.unknown") }))}</span>
...
function formatExpectedRecovery(account) {
  ...
  return account.usage && remainingScore(account) > 0 ? t("detail.availableNow") : t("detail.noExpectedRecovery");
}

function formatImmediateState(account) {
  if (!account.usage) return t("labels.unknown");
  const fiveBlocked = Number(account.usage?.fiveHour?.usedPercent) >= 100;
  const weekBlocked = Number(account.usage?.oneWeek?.usedPercent) >= 100;
...
function switchMessage(prefix, result) {
  ...
  const launch = result.launchedCodex ? t(result.verifiedCodexClosure ? "status.launchOk" : "status.launchFresh") : t("status.launchFailed");
  const warning = result.transportWarning ? ` ${t("status.transportWarning", { error: result.transportWarning })}` : "";
  const login = result.targetNeedsLogin ? ` ${t("status.accountNeedsLogin", { email: result.switchedTo?.emailMasked ?? t("labels.unknown") })}` : "";
  return `${t("status.switchResult", { prefix, count: closed, launch })}${warning}${login}`;
}
...
function reportCacheIsFresh(entry) {
  return Boolean(entry?.updatedAt && Date.now() - entry.updatedAt < clampRefreshInterval(settings.usageRefreshIntervalMinutes) * 60_000);
}
...
async function refreshWeeklyReport({ force = false, background = false } = {}) {
  ...
  if (!force && reportCacheIsFresh(cached)) {
    renderReportsView();
    return;
  }
  ...
  try {
    ...
    const next = mode === "daily"
      ? await api.getDailyUsageReport({ date })
      : await api.getWeeklyUsageReport({ weekStart: date });
    const updatedAt = Date.now();
    reportCache.set(key, { report: next, updatedAt });
...
function renderReportsView() {
  ...
  reportsRefreshState.textContent = weeklyReportLoading
    ? t("reports.updatingCached", { time: weeklyReportUpdatedAt ? new Date(weeklyReportUpdatedAt).toLocaleString() : t("labels.unknown") })
    : weeklyReportUpdatedAt
      ? t("reports.lastUpdated", { time: new Date(weeklyReportUpdatedAt).toLocaleString() })
  ...
  syncReportFilters(report);
  const rows = filteredReportAccounts(report);
  ...
  reportsCapacityAccounts.innerHTML = rows.length ? rows.map((account) => `
    <div class="reports-capacity-account"><strong>${escapeHtml(account.emailMasked)}</strong><span>5h ${formatAccountCapacityEvidence(report?.capacity?.fiveHour, account.accountId)}</span><span>7d ${formatAccountCapacityEvidence(report?.capacity?.oneWeek, account.accountId)}</span></div>
  ...
  if (!rows.length) {
    ...
    reportsAccountTable.innerHTML = rows.map((account) => `
      <tr class="report-account-row">
        <td data-label="${escapeHtml(t("reports.account"))}"><strong>${escapeHtml(account.emailMasked)}</strong><small>${escapeHtml(account.planType)}</small></td>
        <td data-label="${escapeHtml(t("reports.attributedTokens"))}">${formatCompactNumber(account.totalTokens)}</td>
        <td data-label="${escapeHtml(t("reports.composition"))}">${formatTokenComposition(account)}</td>
        <td class="report-usage-overview" data-label="${escapeHtml(t("reports.usageOverview"))}"><strong>${Number(account.sharePercent ?? 0).toFixed(1)}%</strong><small>${escapeHtml(t("reports.sessions"))} ${account.sessionCount ?? 0} · ${escapeHtml(t("reports.averageSession"))} ${formatCompactNumber(account.averageTokensPerSession ?? 0)}</small></td>
        <td data-label="${escapeHtml(t("reports.attributionConfidence"))}">${escapeHtml(t(account.attributionConfidence === "medium" ? "reports.confidenceMedium" : "reports.confidenceHigh"))}</td>
      </tr>
    `).join("");
  }

  const modelRows = rows.flatMap((account) => account.models.flatMap((model) => model.reasoning.map((reasoning) => ({ account, model, reasoning }))));
  reportsModelGroups.innerHTML = modelRows.length ? modelRows.map(({ account, model, reasoning }, index) => `
    <div class="report-model-item">
      <div class="report-model-row">
        <span class="report-model-account" data-label="${escapeHtml(t("reports.account"))}">${escapeHtml(account.emailMasked)}</span>
  ...
  reportsResetHistory.innerHTML = report?.resets?.length ? report.resets.map((reset) => `
    <div class="report-event-row">
      ...
      <time>${escapeHtml(t("reports.detectedAt"))} ${escapeHtml(new Date(reset.atMs).toLocaleString())}</time>
    </div>
  `).join("") : `<p class="reports-empty">${escapeHtml(t(reportMode === "daily" ? "reports.dailyNoData" : "reports.weeklyNoData"))}</p>`;
}
...
function setReportMode(mode) {
  ...
  reportsWeekInput.max = mode === "daily" ? reportDates.daily : reportDates.weekly;
  weeklyReport = undefined;
  weeklyReportUpdatedAt = undefined;
  hydrateReportCache();
  renderReportsView();
...
function syncReportFilters(report) {
  const accounts = (report?.accounts ?? []).map((item) => ({ value: item.accountId, label: item.emailMasked }));
  ...
  populateReportFilter(reportsAccountFilter, accounts, t("reports.all"));
...
function filteredReportAccounts(report) {
  if (!report || report.source !== "local_observation") return [];
  if (reportsConfidenceFilter.value && report.confidence !== reportsConfidenceFilter.value) return [];
  return report.accounts
    .filter((account) => !reportsAccountFilter.value || account.accountId === reportsAccountFilter.value)
    .map((account) => {
      const models = account.models
        .filter((model) => !reportsModelFilter.value || model.model === reportsModelFilter.value)
        .map((model) => ({
          ...model,
          reasoning: model.reasoning.filter((item) => !reportsEffortFilter.value || item.reasoningEffort === reportsEffortFilter.value)
        }))
        .filter((model) => model.reasoning.length > 0);
      if (!reportsModelFilter.value && !reportsEffortFilter.value) return { ...account, models };
    ...
    .filter((account) => account.models.length > 0 || (!reportsModelFilter.value && !reportsEffortFilter.value));
}
...
function formatExpectedResets(value = {}) {
  const five = value.fiveHour ? new Date(value.fiveHour).toLocaleString() : t("labels.unknown");
  const week = value.oneWeek ? new Date(value.oneWeek).toLocaleString() : t("labels.unknown");
...
function isFreshQuotaSnapshot(account) {
  return Boolean(account?.usage?.fetchedAt && Date.now() / 1000 - account.usage.fetchedAt <= 10 * 60 && !account.usageError);
}
